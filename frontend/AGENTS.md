...
| **The suite runs `next dev`, so the dev overlay's obstruction is a CI fact, not just a local one** | `playwright.config.ts`'s `webServer` is `next dev`, and Next's overlay sits exactly over the `MONTH` toggle at 375px. No test before T057 ever clicked `MONTH`, which is why this went unnoticed — `view-week` is clickable and `view-month` is not. `pipeline.spec.ts` uses `dispatchEvent("click")` there with the reason written at the call site. Recorded rather than worked around silently: a control that no automated test can *click* is one restyle away from being broken with the suite still green. |
